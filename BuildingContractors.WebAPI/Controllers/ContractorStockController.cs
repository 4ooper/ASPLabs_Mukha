...
namespace BuildingContractor.WebAPI.Controllers
{
    ...
    public class ContractorStockController : BaseController
    {
        ...
        [HttpGet("[action]/")]
        public async Task<ActionResult> GetSteelAveragePrice()
        {
            var query = new GetContractorStockListQuery();
            var vm = await Mediator.Send(query);
            var record = vm.contractorStock.Where(item => item.conctractorMaterial.name == "Steel").Average(item => item.price);
            return Ok(record);
        }

        [HttpGet("[action]/")]
        public async Task<ActionResult> GetSteelByPrice()
        {
            var query = new GetContractorStockListQuery();
            var vm = await Mediator.Send(query);
            var records = vm.contractorStock.Where(item => item.conctractorMaterial.name == "Steel").Where(item => item.price > 10000);
            return Ok(records); 
        }
