...
namespace BuildingContractor.WebAPI.Controllers
{
    ...
    public class ContractorController : BaseController
    {
        ...
        [HttpGet("[action]/")]
        public async Task<ActionResult<ContractorListVm>> GetBeeween()
        {
            var query = new GetContractorListQuery();
            var vm = await Mediator.Send(query);
            var records = vm.contractors.Where(item => item.id % 2 == 0).ToList();
            vm = new ContractorListVm { contractors = records };
            return Ok(records);
        }
